...
REGISTRY :=ghcr.io/galinko87
VERSION=$(shell git describe --tags --abbrev=0)-$(shell git rev-parse --short HEAD)
TARGETOS=linux
TARGETARCH=amd64 # arm64

define code_builder
	CGO_ENABLED=0 GOOS=$1 GOARCH=$2 go build -v -o kbot -ldflags "-X="github.com/galinko87/kbot/cmd.appVersion=${VERSION}
endef
...
build: format get
	CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -v -o kbot -ldflags "-X="github.com/galinko87/kbot/cmd.appVersion=${VERSION}
